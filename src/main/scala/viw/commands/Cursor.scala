...
object Cursor {
  ...
  object MoveUp extends CommandRunner {
    override def conditions: List[(State) => Boolean] = List(StateMode.isViwOn, not(LinePosition.firstLine))
  ...
  object MoveLeft extends CommandRunner {
    override def conditions: List[(State) => Boolean] = List(StateMode.isViwOn, not(LinePosition.lineStart))
  ...
  object MoveToStartOfNextWord extends CommandRunner{
    override def conditions: List[(State) => Boolean] = List(StateMode.isViwOn, not(LinePosition.endOfFile))
